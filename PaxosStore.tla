...
Message == [type: {"Prepare", "Accept", "ACK"}, 
            from: Participant, to : SUBSET Participant, 
...
Accept(p, b, v) == 
    \* TODO: delete it? to allow duplication?
    /\ ~ \E m \in msgs : m.type = "Accept" /\ m.state[p].maxBal = b 
    /\ b \in Bals(p)    \* TODO: delete it?
    /\ \E Q \in Quorum : \A q \in Q : state[p][q].maxBal = b \* TODO: majority quorum (local ack?)
